...
around detect => sub {
    ...
    if (is_string($arg1) && (is_value($arg2) || is_regex_ref($arg2))) {
...
# around select replaces getByFieldValue
sub getByFieldValue {
...
# pluck replaces selectField
sub selectField {
	my ($self, $fieldname) = @_;
	return $self->collection->find->fields({$fieldname => 1})->all;
}
