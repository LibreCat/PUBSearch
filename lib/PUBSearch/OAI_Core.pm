package PUBSearch::OAI_Core; # TODO deletedRecord=persistent, hierarchical sets, setDescription
...
sub oai_provider {
    ...
    #my $fixer = Catmandu::Fix->new (fixes => $format->{fix});
    ...
    any ['get', 'post'] => $path => sub {
        ...
        content_type 'xml';
		my $fixer = Catmandu::Fix->new (fixes => $format->{fix}) if $format->{fix};
        if ($verb eq 'GetRecord') {
            ...
            if (my $rec = $bag->get($id)) {
                ...
                my $oaiRec = $fixer->fix($rec);
                $vars->{metadata} = template $format->{template}, $oaiRec;
            ...
            my @cql;
			push @cql, "($format->{filter})"       if $format->{filter};
            ...
            unless (@cql) {
            	#quickfix, since cql.allRecords does not work yet!
                #push @cql, "(cql.allRecords)"; 
                use Date::Simple qw(today);
				my $date = today() +1;
				push @cql, "(datelastchanged <= $date)";
            }
            ...
            if ($verb eq 'ListIdentifiers') {
                ...
                $vars->{records} = [map {
                    ...
                    unless ($deleted) {
                    	my $oaiRec = $fixer->fix($rec);
                        $metadata = template $format->{template}, $oaiRec;
                    }
                    {
                        id => $rec->{_id},
                        datestamp => _combined_utc_datestamp($rec->{$setting->{datestamp_field}}),
